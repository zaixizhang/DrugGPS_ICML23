...
def ligand_gen(batch, model, vocab, config, center):
    pos_list = []
    feat_list = []
        ...
        if i == 0:
            ...
            if torch.sum(rotatable) > 0 and i >= 2:
                ...
                alpha = model.forward_alpha(protein_pos=batch['protein_pos'].float(),
                                            ...
                                            ligand_atom_feature=torch.cat(feat_list, dim=0).float(),
                                            batch_protein=batch['protein_element_batch'],
                                            batch_ligand=ligand_batch, xy_index=torch.tensor(xy_index), rotatable=rotatable, batch=batch)
                ...
                for j in range(len(alpha)):
                    ...
                    xn_pos = rand_rotate((positions[x_index[j]] - positions[y_index[j]]).reshape(-1), positions[x_index[j]].reshape(-1),
                                         xn_pos, alpha[j])
                    if xn_pos.shape[0]>0:
