...
class LigandBFSMask(object):
    ...
    def __call__(self, data):
        ...
        if set(data['moltree'].nodes[bfs_perm[-num_masked]].clique).intersection(set(data['ligand_inter_atoms']))!=set():
            data['interaction'] = torch.tensor([1.])
        else:
            data['interaction'] = torch.tensor([0.])
